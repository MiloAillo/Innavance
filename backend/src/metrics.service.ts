  ...
  async update() {
    ...
    await this.prisma.$transaction(
      occupiedRoomsId.map((room) => {
        ...
        const smartDoorIsOpened = isSmartDoorLocked
          ? false
          : booleanRandomizer({ true: 50, false: 50 });
        const electricityOutput = floatRandomizer([
          { range: [8, 10], weight: 85 },
          { range: [11, 13], weight: 15 },
        ]);
        const isWaterFlowing = booleanRandomizer({ true: 10, false: 90 });
        const waterOutput = isWaterFlowing
          ? floatRandomizer([
              { range: [6, 8], weight: 85 },
              { range: [9, 11], weight: 15 },
            ])
          : 0;
    ...
    await this.prisma.$transaction(
      nonOccupiedRoomsId.map((room) => {
        ...
        const smartDoorIsOpened = isSmartDoorLocked
          ? false
          : booleanRandomizer({ true: 100, false: 0 });
        const electricityOutput = floatRandomizer([
          { range: [0.1, 0.3], weight: 80 },
          { range: [0.4, 0.5], weight: 20 },
        ]);
