...
type RoomWithBookings = Prisma.RoomsGetPayload<{
  include: {
    bookings: {
      where: { status: { in: ['checked_in', 'checking_out'] } };
      include: {
...
export class DashboardGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext) {
    ...
    if (!roomId)
      throw new UnauthorizedException('room id is missing in url param');
    if (typeof roomId === 'object')
      throw new BadRequestException('room id request is broken');
