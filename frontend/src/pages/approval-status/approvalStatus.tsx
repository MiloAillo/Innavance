...
export function ApprovalStatus(): JSX.Element {
    const [ state, setState ] = useState<"FETCH_STATUS" | "NO_STATUS" | "STATUS_FETCHED" >("FETCH_STATUS")
    ...
    (
        <motion.div 
            ...
            {...FRAMER_ANIMATION}
        >
            <p className="font-medium text-xl text-black/65">{bookingData?.room_name} Approval Status</p>
            <div className="flex flex-col md:flex-row gap-10 md:gap-0">
                <div className="flex-1">
                    {/* status */}
                    {(bookingData?.status === "checked_in" || bookingData?.status === "checking_out") &&
                    ...
                    {bookingData?.status === "on_hold" &&
                        <p className="font-semibold text-2xl text-orange-500">Waiting for Approval</p>
                    }
                    {bookingData?.status === "rejected" &&
                        <p className="font-semibold text-2xl text-red-500">Rejected</p>
                    }
                    {/* auto approve countdown if existed */}
                    {bookingData?.is_auto_approve && bookingData.status === "on_hold" && countdown &&
                        <p className="font-normal text-lg">Auto approve in {countdown}</p>
                    }
                    {/* notice */}
                    {bookingData?.status === "on_hold" &&
                        <p className="text-sm pt-3">Once reviewed, we will contact your phone number. Make sure to check for incoming messages.</p>
                    }
                    {(bookingData?.status === "checked_in" || bookingData?.status === "checking_out") &&
                        <p className="text-sm pt-3">We have sent you the door PIN and the account ID for you to enter the room dashboard.</p>
                    }
                    {bookingData?.status === "rejected" &&
                        <p className="text-sm pt-3">You may speak with our staff for a solution. Until then, you should not retry to reserve another room.</p>
                    }
                </div>
                <div className="flex-1 text-end flex flex-col items-end">
                    ...
                    <div className="flex flex-col gap-1">
                        <p className="font-medium text-sm text-neutral-500">Payment Method</p>
                    ...
                    <div className="w-max flex flex-col gap-1 pt-7">
                        ...
                        <p className="font-medium text-base text-neutral-500 pt-1">Full Name</p>
    ...
    return (
        <section className="bg-neutral-100 flex justify-center items-center w-screen min-h-screen flex-col gap-5">
            ...
            <motion.div 
                layout
                transition={{ type: "spring", stiffness: 300, damping: 30, mass: 2 }}
                className="px-5 py-4 rounded-lg bg-white shadow flex justify-center items-center mx-7 md:mx-10"
            >
